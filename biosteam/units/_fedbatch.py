...
class FedBatch(BatchBioreactor):
    """
    ...
    def _calc_efficiency(self, feed, tau, V): # pragma: no cover
        # Get initial concentrations
        y, e, s, o = [17, 1, 2, 8]#feed.indices(['Yeast',   '64-17-5','492-61-5', '7732-18-5'])
        mass = feed.mass
        F_vol = feed.F_vol
        concentration_in = mass/F_vol
        X0, P0, S0, O20 = (concentration_in[i] for i in (y, e, s, o))
        
        # Integrate to get final concentration
        ag=(113.4, 0.45, 0.18, 0.31, 1.01, 1.88, 2.81, 82.8, 108.2, feed.T, F_vol, V, S0)

        t = np.linspace(0, tau, 1000)
        C_t = odeint(self.kinetic_model, (X0, P0, S0, O20), t, args=ag)
        # Cache data
        self._X = C_t[:, 0]
        self._P = C_t[:, 1]
        self._S = S = C_t[:, 2]
    ...
    @staticmethod
    def kinetic_model(z, t, T, F, V, *kinetic_constants): # pragma: no cover
        """
        ...
        *kinetic_constants
        ...
        #mu_m1, mu_m2, Ks1, Ks2, Pm1, Pm2, Xm, Y_PS, a, T, F_vol, V = kinetic_constants
        mu_m1, mu_m2, Ks1, Ks2, Pm1, a, T, F_vol, V, Sf = kinetic_constants
    ...
    def _run(self):
        ...
        if self.iskinetic:
            self.fermentation_reaction.X = self._calc_efficiency(effluent, self._tau, self.V)
